...
- **El ritmo importa más que el volumen.** X declara su límite en cada respuesta: **500 peticiones cada 15 minutos** para este endpoint, y cada perfil consultado gasta una. Con los valores por defecto —3 perfiles por ronda cada 4 h— son 3 de esas 500, así que el tope no es lo que aprieta. Lo que sí importa es que `objetivos_por_ronda` y el intervalo del scheduler **se multiplican** y se configuran en pantallas distintas: la TUI enseña el número que sale con tu configuración y `scrappy doctor` avisa si la combinación se acerca al tope. Además, Scrappy se retira solo en cuanto X responde con un límite.
